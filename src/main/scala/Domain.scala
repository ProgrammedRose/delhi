...
// =============================================================================
// Блок 3: State — изменяемое состояние торгового автомата
// =============================================================================
// Файл содержит:
//   1. MachineState       — снимок всего состояния автомата в момент времени
//   2. VendingMachine     — переходы состояния: insertCoin, selectProduct,
//                           cancelPurchase, refillProduct
//
// Ключевая идея State:
...
//   — Reader (Блок 1) используется ВНУТРИ переходов: мы читаем конфигурацию
//     (допустимые монеты, лимит, цены) чтобы принять решение о переходе.
//     Для этого функции принимают cfg: VendingConfig явным параметром —
//     самый простой способ «протащить» конфиг без трансформеров монад.
//   — Writer (Блок 2) используется для возврата лога вместе с результатом:
//     каждый переход возвращает (новое состояние, лог) через W[Result[...]].
//
// Почему конфиг — явный параметр, а не трансформер?
//   Комбинировать State и Reader «правильно» означало бы писать
//   StateT[[A] =>> Reader[VendingConfig, A], MachineState, A] — это сложно
//   и задание явно говорит «лучше без трансформеров».
//   Передача cfg явно — честный, читаемый и полностью функциональный подход.
// =============================================================================
...
// Это «S» в State[MachineState, A].
//
// Все поля неизменяемые (val в case class).
// «Изменение» состояния — это создание нового объекта через .copy(поле = значение).
// Scala делает это эффективно: copy создаёт новый объект только с изменёнными полями.
final case class MachineState(

                               // Остатки товаров на складе: название → количество единиц.
                               // Например: Map("cola" -> 5, "water" -> 3, "juice" -> 0, "coffee" -> 2)
                               // Отсутствие ключа и значение 0 семантически одинаковы — товара нет.
                               stock: Map[String, Int],

                               // Сумма монет, внесённых покупателем прямо сейчас (до завершения транзакции).
                               // Обнуляется при успешной покупке или отмене.
                               // В копейках, как и все суммы в этом проекте.
                               inserted: Int,

                               // Накопленная выручка автомата — сумма всех успешных покупок.
                               // Растёт при каждой успешной покупке на величину (цена - сдача).
                               // Точнее: увеличивается на price, уменьшается на change при выдаче.
                               revenue: Int,

                               // Касса автомата: сколько монет каждого номинала сейчас внутри.
                               // Используется при расчёте сдачи: нельзя выдать монету, которой нет.
                               // Например: Map(100 -> 10, 50 -> 5, 10 -> 20) — 10 монет по рублю и т.д.
                               cashBox: Map[Int, Int]
                             )

object MachineState:
  // Начальное состояние автомата при запуске.
  // Склад заполнен, касса содержит стартовый набор монет для сдачи,
  // внесённая сумма и выручка — нули.
...
//
// Тип результата каждого перехода:
//   State[MachineState, W[Result[...]]]
//
//   — State[MachineState, _]  : переход изменяет состояние автомата
//   — W[...]                  : результат сопровождается Writer-логом
//   — Result[...]             : операция может не удаться (Left = причина отказа)
//
// Такой тип честно выражает все три возможных «эффекта» операции.
...
object VendingMachine:
  ...
  // ---------------------------------------------------------------------------
  // insertCoin: внести монету в автомат.
  //
  // Параметры:
  //   coin — номинал монеты в копейках
  //   cfg  — конфигурация (нужна для проверки допустимости номинала и лимита)
  //
  // Логика:
  //   1. Проверяем, принимает ли автомат этот номинал (canAcceptCoin из Reader).
  //   2. Проверяем, не превысит ли монета лимит внесённой суммы (canInsertMore).
  //   3. При успехе: добавляем монету к inserted и в cashBox.
  //   4. При отказе: состояние не меняем, возвращаем объяснение отказа.
  //
  // Возвращает SM[Int] — новую суммарную внесённую сумму (или ошибку).
  // ---------------------------------------------------------------------------
  def insertCoin(coin: Int, cfg: VendingConfig): SM[Int] =
    // State.get возвращает State[MachineState, MachineState] —
    // «прочитай текущее состояние и верни его как значение».
    // flatMap разворачивает: s — это текущий MachineState.
    State.get[MachineState].flatMap(s =>
      ...
      if !accepted then {
        ...
        // cashBox.updated(coin, n+1): добавляем монету в кассу.
        // getOrElse(coin, 0): если монеты такого номинала ещё не было — начинаем с 0.
  ...
  // Параметры:
  //   product — название товара
  //   hour    — текущий час (для расчёта скидки)
  //   cfg     — конфигурация
  //
  // Логика:
  //   1. Проверяем наличие товара на складе.
  //   2. Рассчитываем цену со скидкой (effectivePrice из Reader).
  //   3. Рассчитываем сдачу из кассы (calculateChangeFromCashBox — учитывает
  //      реальные монеты в кассе, а не просто номиналы из конфига).
  //   4. При успехе: уменьшаем stock, обнуляем inserted,
  //                  увеличиваем revenue, вычитаем монеты сдачи из cashBox.
  //   5. При отказе: состояние не меняем.
  //
  // Возвращает SM[Change] — список монет сдачи (или ошибку).
  // ---------------------------------------------------------------------------
      ...
      if stockCount <= 0 then
        ...
        (basePriceResult, finalPriceResult) match
          ...
          case (Right(basePrice), Right(finalPrice)) =>
            ...
            // Нас интересует только лог — поэтому берём ._2 из run.
  ...
  //
  // Логика:
  //   1. Запоминаем текущую внесённую сумму.
  //   2. Обнуляем inserted в состоянии.
  //   3. Возвращаем сумму покупателю (в реальном автомате — физически,
  //      здесь — как информация в логе и результате).
  //
  // Возвращает SM[Int] — возвращённую сумму.
  //
  // Примечание: монеты возвращаются «обратно» — убираем их из кассы.
  // Мы не восстанавливаем точный состав монет (не помним какие именно
  // вносились), поэтому просто уменьшаем кассу на возвращаемую сумму
  // жадным алгоритмом по тем же номиналам что есть в кассе.
  // ---------------------------------------------------------------------------
  ...
  // Параметры:
  //   product — название товара
  //   amount  — сколько единиц добавить
  //   cfg     — конфигурация (проверяем что товар вообще известен автомату)
  //
  // Логика:
  //   1. Проверяем что товар есть в прайсе (иначе — неизвестный товар).
  //   2. Проверяем что amount > 0 (нельзя «пополнить» на 0 или минус).
  //   3. Добавляем к текущему остатку.
  //
  // Возвращает SM[Int] — новый остаток товара.
  // ---------------------------------------------------------------------------
  ...
  //
  // Отличие от calculateChange (Блок 1):
  //   Блок 1 проверяет только допустимые номиналы из конфига.
  //   Здесь мы смотрим на cashBox: сколько монет каждого номинала есть физически.
  //   Нельзя выдать монету если её счётчик равен 0.
  //
  // Алгоритм — жадный с учётом количества:
  //   Берём номиналы по убыванию, для каждого берём столько монет
  //   сколько нужно (но не больше чем есть в кассе).
  //
  // Приватная — внутренняя деталь реализации VendingMachine.
  // ---------------------------------------------------------------------------
  private def calculateChangeFromCashBox(
    ...
    if amount < 0 then
      ...
      // Берём только те, которых есть хотя бы 1 штука (count > 0).
      ...
      // remaining  — сколько ещё нужно набрать
      // coins      — оставшиеся доступные номиналы (с их количеством)
      // acc        — уже набранные монеты
