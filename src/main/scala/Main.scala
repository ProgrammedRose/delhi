...
object ScenarioStep:
  ...
  // Завёрнуто в IO - это сайд эффект (обращение к системному времени).
  val getCurrentHour: IO[Int] =
    IO(() => java.time.LocalTime.now().getHour)


// -----------------------------------------------------------------------------
// 3. APPSTATE - мост между чистым State и IO
// -------------------------
...
//
// Это единственное место во всём этом жутком проекте где есть var.
