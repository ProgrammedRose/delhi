...
trait Monad[M[_]]:
  ...
  // Пример для Option: pure(42) == Some(42)
  def pure[A](a: A): M[A]
...
//
// Пример: priceOf("cola") возвращает Reader[Config, Int],
//         а не Int напрямую. Цену «достанем» когда запустим с конкретным Config.
...
//
// Ключевая идея: каждый шаг вычисления добавляет записи в лог,
// а flatMap автоматически объединяет логи всех шагов.
// В итоге у нас и результат, и полная история как он получился.
//
// Log должен уметь «объединяться» — нам нужна операция ++.
// Мы используем Vector[String]: быстрое добавление в конец, удобный вывод.
...
// Как я понял - «дай мне старое состояние - верну результат и новое состояние».
//
// Ключевая идея здесь:
...
//
//
// Пример: insertCoin(coin) — это State[MachineState, Unit],
//         который «знает» как обновить сумму внесённых монет.
...
object State:
  ...
  // Самый удобный вариант для точечных обновлений.
  // Например: State.modify(s => s.copy(balance = s.balance + coin))
...
// через unsafeRun. Всё остальное — чистые функции.
//
// «unsafe» в названии — честное предупреждение: здесь происходит
// реальный I/O, и это «небезопасно» с точки зрения чистоты FP.
