// =============================================================================
// Блок 0: Инфраструктура монад
// =============================================================================
// Файл содержит:
//   1. trait Monad[M[_]]          — общий интерфейс для всех монад
//   2. Reader[Env, A]             — чтение из окружения/конфигурации
//   3. Writer[Log, A]             — накопление лога рядом с вычислением
//   4. State[S, A]                — моделирование изменяемого состояния
//   5. IO[A]                      — описание побочных эффектов (ввод-вывод)
// =============================================================================

// -----------------------------------------------------------------------------
// 1. TRAIT MONAD
// -----------------------------------------------------------------------------
// Монада — это абстракция над «вычислением с эффектом».
// M[_] — тип более высокого порядка (higher-kinded type):
//   не просто Int или String, а «контейнер», который принимает тип как параметр.
//   Примеры: Option[_], List[_], наш Reader[Env, _] и т.д.
//
// Три ключевые операции:
//   pure     — заворачивает чистое значение в контекст монады.
//              Аналог конструктора: «вот значение, оберни его».
//   flatMap  — цепочка: берёт значение из контекста, применяет функцию,
//              получает новый контекст. Это и есть «монадическое связывание».
//   map      — трансформация значения внутри контекста без смены самого контекста.
//              Выражается через flatMap + pure, поэтому не абстрактная.
